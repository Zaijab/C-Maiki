...
group_eigen_hat = [np.linalg.eig(laplacian_hat)
                   for laplacian_hat in group_laplacian_hat]
for i, eigen in enumerate(group_eigen_hat):
    eigenvalues, eigenvectors = eigen
    plt.figure()
    plt.title(f"Plantroot")
    plt.scatter(eigenvectors[1], eigenvectors[2])
    plt.savefig(f"{figure_path}/two_eigenvectors_plantroot.pdf")
    plt.close()
np.random.seed(0)
